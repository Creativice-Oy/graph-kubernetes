...
import { createContainerSpecEntity, createDeploymentEntity } from './converters';
      ...
      await client.iterateNamespacedDeployments(
        ...
        async (deployment) => {
          ...
          for (const container of deployment.spec?.template.spec?.containers || []) {
            const containerSpecEntity = createContainerSpecEntity(deployment.metadata?.uid!, container)
...
export const deploymentsSteps: IntegrationStep<IntegrationConfig>[] = [
  {
    ...
    relationships: [Relationships.NAMESPACE_CONTAINS_DEPLOYMENT, Relationships.DEPLOYMENT_USES_CONTAINER_SPEC],
